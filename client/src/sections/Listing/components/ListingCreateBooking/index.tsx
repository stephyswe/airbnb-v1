...
export const ListingCreateBooking = ({
  ...
  const disabledDate = (currentDate?: Moment, checkOut?: Boolean) => {
    if (currentDate) {
      // currentDate being assessed is before the end of today.
      const dateIsBeforeCheckIn = checkOut ? moment(currentDate).isBefore(checkInDate, "days") : false
      const dateIsBeforeEndOfDay = currentDate.isBefore(moment().endOf("day"));
      const dateIsMoreThenThreeMonthsAhead = moment(currentDate).isAfter(moment().endOf("day").add(90, "days"))
      
      return dateIsBeforeEndOfDay || dateIsBeforeCheckIn || dateIsMoreThenThreeMonthsAhead || dateIsBooked(currentDate);
    } else {
      return false;
    }
  };
        ...
        <div>
          ...
          <div className="listing-booking__card-date-picker">
            ...
            <DatePicker
              ...
              disabledDate={c => disabledDate(c, true)}
              onChange={(dateValue) => verifyAndSetCheckOutDate(dateValue)}
              dateRender={current => {
                if (moment(current).isSame(checkInDate ? checkInDate : undefined, "day")) {
                  return (
                    <Tooltip title="Check in date">
                      <div className="ant-clendar-date ant-calendar-date__check-in">{current.date()}</div>
                    </Tooltip>
                  )
                } else {
